...
app.post("/api/UpdateUserData", function (req, res) {
    var mod = new User(req.body);
    console.log("data for update" + JSON.stringify(mod));
    User.findByIdAndUpdate(req.body._id, {name:req.body.name,gender:req.body.gender,email:req.body.email, mobile: req.body.mobile, address: req.body.address, address2: req.body.address2, city: req.body.city , state:req.body.state, zip:req.body.zip},
        function (err, docs) {
            if (err) {
                console.log(err)
            }
...
app.post("/api/updateReviews", function (req, res) {
    var mod = new allReview(req.body);
    console.log("data for update" + JSON.stringify(mod));
    allReview.findByIdAndUpdate(req.body._id, { ratings:req.body.ratings,comments:req.body.comments},
        function (err, docs) {
            if (err) {
                console.log(err)
            }
